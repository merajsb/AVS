...
public class GaussianFilter extends ConvolveFilter {
    ...
    public BufferedImage filter( BufferedImage src, BufferedImage dst ) {
        ...
        convolve(kernel, inPixels, outPixels, width, height, alpha, CLAMP_EDGES);
    ...
    public static void convolveAndTranspose(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, boolean alpha, int edgeAction) {
        ...
        for (int y = 0; y < height; y++) {
            int index = y;
            //TODO: read convolve summation on that page
            int ioffset = y*width;
    ...
    public static Kernel makeKernel2D(float radius) {
        int r = (int)Math.ceil(radius);
        int rows = r*2+1;
